...
<div>
    <x-header title="Edit Task" separator />
    <div class="max-w-[700px] mx-auto">
        <x-form wire:submit="save">
            ...
            <x-choices label="Assing employer" wire:model="assigned_to" :options="$project->employers" {{-- option_value= "user_id" --}}
                value="user_id" no-result-text="Ops! Nothing here ..." single>
                {{-- Item slot --}}
                @scope('item', $user)
            ...
            <x-range wire:model.live="task_points" label="Task points" min=1 max=10 />
            <x-badge :value="$task_points" class="badge-neutral" />
            <x-slot:actions>
                <x-button label="Cancel" class="btn-ghost" link="/projects" />
                <x-button label="Update" type="submit" icon="o-paper-airplane" class="btn-warning"
                    spinner="register" />
            </x-slot:actions>
        </x-form>
    </div>
