...
new class extends Component {
    ...
    {
        return [['key' => 'title', 'label' => 'Title', 'class' => 'w-40'], ['key' => 'due_date', 'label' => 'Due date', 'class' => 'w-32'], ['key' => 'estimated_completion_time', 'label' => 'Estimate (hrs)', 'class' => 'w-20'], ['key' => 'assigned_to', 'label' => 'Employer', 'class' => 'w-32'], ['key' => 'status', 'label' => 'Status', 'class' => 'w-10']];
    }
        ...
        <div class="max-w-xl pl-8">
            <form wire:submit="add_member">
                @if (auth()->user()->role == 'manager')
                    <x-choices wire:model="selectedUserIds" :options="$usersSearchable" search-function="search"
            ...
            @forelse ($project->employers as $employer)
                <x-list-item :item="$employer">
                    <x-slot:avatar>
                        <img class="rounded-full w-12" src="{{ $employer->avatar ?? '/empty-user.jpg' }}"
                            alt="employer avatar">
                    </x-slot:avatar>
                    <x-slot:value>
                        {{ $employer->full_name }}
                    ...
                    @if (auth()->user()->role == 'manager')
                        <x-slot:actions>
                            <x-button icon="o-trash" class="text-red-500"
                                wire:click="deleteMember({{ $employer['id'] }})" wire:confirm="Are you sure?" spinner />
                        </x-slot:actions>
                    @endif
                </x-list-item>
                        ...
                        @scope('actions', $task, $project)
                            @if (auth()->user()->role == 'manager')
                                <div class="flex flex-nowrap gap-2">
                                    <x-button link="/projects/{{ $project->id }}/tasks/{{ $task->id }}/edit"
                                        icon="o-pencil" class="btn-sm btn-ghost" />
                            ...
                            @switch($task->status)
                                @case('pending')
                                    <x-badge value="{{ $task->status }}"
                                        class="capitalize font-bold badge badge-outline badge-warning" />
                                @break

                                @case('in_progress')
                                    <x-badge value="{{ $task->status }}"
                                        class="capitalize font-bold badge badge-outline badge-primary" />
                                @break

                                @case('completed')
                                    <x-badge value="{{ $task->status }}"
                                        class="capitalize font-bold badge badge-outline badge-success" />
                                @break
